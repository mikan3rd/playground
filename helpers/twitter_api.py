...
class TwitterApiClient:
    def __init__(self, access_token, secret):
        ...
        self.api = OAuth1Session(
            TWITTER_CONSUMER_KEY, TWITTER_CONSUMER_SECRET, access_token, secret
        )
    ...
    def get_tweet(self, tweet_id, tweet_mode="extended"):
        endpoint = "https://api.twitter.com/1.1/statuses/show.json"
        params = {"id": tweet_id, "tweet_mode": tweet_mode}
        response = self.api.get(endpoint, params=params)
        return json.loads(response.text)

    def get_home_timeline(
        self, count=200, trim_user=False, exclude_replies=True, include_entities=False
    ):
    ...
    def get_retweet_user(self, tweet_id, count=100, trim_user=False):
        endpoint = "https://api.twitter.com/1.1/statuses/retweets/%s.json" % (tweet_id)
        params = {"count": count, "trim_user": trim_user}
        response = self.api.get(endpoint, params=params)
        return json.loads(response.text)

    def get_retweeter_id(self, tweet_id, count=100, cursor=None, stringify_ids=True):
    ...
    def get_friendships(self, user_id=None, screen_name=None):
    ...
    def post_follow(self, screen_name=None, user_id=None, follow=False):
        endpoint = "https://api.twitter.com/1.1/friendships/create.json"
        params = {"screen_name": screen_name, "user_id": user_id, "follow": follow}
        response = self.api.post(endpoint, params=params)
        return json.loads(response.text)

    def post_unfollow(self, screen_name=None, user_id=None):
